...
class DecisionSimulatorController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $totalCardDebt = (float) DB::table('cards')
            ->where('user_id', $user->id)->sum('current_debt');
        ...
        $current = [
            ...
            'avg_monthly_expense'=> round((float) $avgMonthlyExpense, 2),
            'monthly_savings'    => round($monthlyIncome - (float) $avgMonthlyExpense, 2),
            'savings_rate'       => $monthlyIncome > 0
                ? round(($monthlyIncome - (float) $avgMonthlyExpense) / $monthlyIncome * 100, 1)
                : 0,
            ...
            'months_emergency'   => $avgMonthlyExpense > 0
                ? round($totalBalance / (float) $avgMonthlyExpense, 1)
                : 0,
        ];

        return view('decision-simulator.index', compact('current'));
    }
    ...
    public function calculate(Request $request): JsonResponse
    {
        ...
        $base_income   = (float) $request->monthly_income;
        $base_expense  = (float) $request->avg_monthly_expense;
        ...
        $newIncome   = $base_income  * (1 + $incomePct);
        $newExpense  = $base_expense * (1 + $expensePct);
        $newSavings  = $newIncome - $newExpense;
        $savingsRate = $newIncome > 0 ? $newSavings / $newIncome : 0;

        // Monthly nominal savings compounding
